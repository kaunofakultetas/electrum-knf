...
if [ ! -z "$ELECBUILD_COMMIT" ] ; then
    ...
    FRESH_CLONE="$CONTRIB_WINE/fresh_clone/electrum-knf" && \
        rm -rf "$FRESH_CLONE" && \
        umask 0022 && \
...
docker run -it \
    ...
    -v "$PROJECT_ROOT_OR_FRESHCLONE_ROOT":/opt/wine64/drive_c/electrum-knf \
    --rm \
    --workdir /opt/wine64/drive_c/electrum-knf/contrib/build-wine \
